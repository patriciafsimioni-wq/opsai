...
export function buildRentalInvoiceReminderEmail(params: {
  ...
  const { managerName, stationLabel, appUrl } = params;

  const subject = "Action needed: report your rental vehicles & upload invoices";
  ...
  <p style="font-size: 14px; line-height: 1.6;">
    If you currently have any <strong>rental vehicles</strong>${stationLabel ? ` at <strong>${escapeHtml(stationLabel)}</strong>` : ""},
    please report them in the portal and <strong>upload the invoices right away</strong>. This is a reminder to
    submit any outstanding rentals and invoices <strong>immediately</strong> so we stay current.
  </p>
  <p style="font-size: 14px; line-height: 1.6;">
    Open the Rental Vehicles page, add each rental vehicle (if it isn't there yet), and attach the invoice —
    including the <strong>amount</strong> and a <strong>photo or PDF</strong>. If you have no rentals right now, no action is needed.
  </p>
  <div style="text-align: center; margin: 24px 0;">
    <a href="${appUrl}/rental-vehicles" style="display: inline-block; background: #d97706; color: white; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">Report Rentals & Upload Invoices</a>
  </div>`,
  );
